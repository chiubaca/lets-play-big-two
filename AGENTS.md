...
- macOS with Homebrew
- Admin access (for `/etc/hosts` setup)
...
This installs caddy and mkcert via Homebrew, then configures:

- `/etc/hosts` entries for `local.bigtwo.com` and `local.api.bigtwo.com`
- Local HTTPS certificates via mkcert
- Caddy reverse proxy
...
caddy run --config Caddyfile   # Reverse proxy (separate terminal)
